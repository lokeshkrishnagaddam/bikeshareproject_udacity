...
def display_data(df, current_line):
    '''Displays data and ask the user if they would like to see five more.
    ...
    Returns:
        (list) with two values:
            ...
            Second value: String stating the name of the most popular end station
                ...
                that accounted for
    '''
    s_sta_counts = df.groupby('Start Station')['Start Station'].count()
    e_sta_counts = df.groupby('End Station')['End Station'].count()
    sor_sta_stations = s_sta_counts.sort_values(ascending=False)
    sor_end_stations = e_sta_counts.sort_values(ascending=False)
    to_tr = df['Start Station'].count()
    mo_pop_sta_station = "\nMost pop start stat: " + sor_sta_stations.index[0] + " (" + str(sor_sta_stations[0]) + " trips, " + '{0:.2f}%'.format(((sor_sta_stations[0]/to_tr) * 100)) + " of trips)"
    mo_pop_end_station = "Most pop end stat: " + sor_end_stations.index[0] + " (" + str(sor_end_stations[0]) + " trips, " + '{0:.2f}%'.format(((sor_end_stations[0]/to_tr) * 100)) + " of trips)"
    return [mo_pop_sta_station, mo_pop_end_station]
...
def statistics():
    ...
    def restart_question():
        ...
        if restart.lower() == 'yes':
             ...
             print("\nNot sure if you wanted to restart or not. Let's try again.")
